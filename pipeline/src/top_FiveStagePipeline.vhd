...
architecture behavior of top_FiveStagePipeline is 
    ...
    -- HAZARD
    signal s_load_hazard, s_pc_write_en, s_if_id_write_en, s_id_ex_flush, s_stall, s_if_id_flush, s_branch_taken, s_jump_taken : std_logic;
    ...
    signal s_forward_data_mem : std_logic_vector(1 downto 0); -- 00=EX_MEM, 01=EX/
...
begin
    ...
    inst_IF_ID: IF_ID
        port map(
            clock    => clock,
            reset    => reset or s_if_id_flush or s_jump_taken,
            write_en => s_if_id_write_en,
            pc       => s_pc_mais_1,
    ...
    inst_HazardDetectionUnit: HazardDetectionUnit
        port map(
            id_ex_mem_to_reg => s_mem_to_reg_idex,
            id_ex_rd         => s_rd_idex,
            if_id_rs         => s_inst_ifid(11 downto 8),
            if_id_rt         => s_inst_ifid(7 downto 4),
            pc_write_en      => s_pc_write_en,
            if_id_write_en   => s_if_id_write_en,
            stall            => s_stall
        );
