...
architecture behavior of top_SinglecycleProcessor is 
    ...
    -- REGISTRADORES DE PIPELINE (Assumindo Reset Ativo-Alto)
    component IF_ID is
     port(
    ...
    -- EX/ULA
    signal s_ula_entry_a, s_ula_entry_b, s_ula_entry_b_final : std_logic_vector(15 downto 0); 
    ...
    signal s_rd_exmem : std_logic_vector(3 downto 0); 
...
begin
    ...
    -- =================================================================================
    -- 3. LÓGICA DE HAZARD DETECTION (FLUSH) E PC
    -- =================================================================================

    -- IMPLEMENTAÇÃO DO FLUSH (Branch Flush Logic)
    s_id_ex_flush <= '1' when (s_branch_e_exmem = '1' and s_ula_zero = '1') or  -- Branch if Equal (BEQ)
                              (s_branch_ne_exmem = '1' and s_ula_zero = '0') else '0'; -- Branch if Not Equal (BNE)


    -- LÓGICA DE MUX DO PRÓXIMO PC
    process(s_pc_mais_1, s_ula_zero, s_imm_ext_idex, s_jump_idex, s_branch_e_idex, s_branch_ne_idex, s_ula_out_exmem)
    begin        
        if s_jump_idex = '1' then
            -- JUMP: O endereço de Jump é o PC atual + Offset
            s_pc_next <= s_pc_mais_1;
        elsif (s_branch_e_exmem = '1' and s_ula_zero = '1') or 
              (s_branch_ne_exmem = '1' and s_ula_zero = '0') then
            -- BRANCH TOMADO: Endereço-alvo do branch (PC+1 + Imediato/Offset)
            s_pc_next <= s_pc_mais_1 + s_ula_out_exmem(7 downto 0); 
        else
            s_pc_next <= s_pc_mais_1;
        end if;
    end process;
    ...
    -- AUXILIARES
    s_imediato_ext <= x"000" & s_inst_ifid(3 downto 0); -- Extensão do imediato (4 para 16 bits)

    
    -- =================================================================================
    -- 4. LÓGICA DE FORWARDING (Encaminhamento de Dados)
    -- =================================================================================
    
    -- UNIDADE DE FORWARDING: Determina se o dado deve ser encaminhado para a ULA
    
    -- Determina o sinal de forward_a (rs)
    
    s_forward_a <= "01" when (s_reg_write_exmem = '1' and s_rd_exmem /= "0000" and s_rd_exmem = s_rs_idex) else
		   "10" when (s_reg_write_memwb = '1' and s_rd_memwb /= "0000" and s_rd_memwb = s_rs_idex) else
		   "00";
				   
    s_forward_b <= "01" when (s_reg_write_exmem = '1' and s_rd_exmem /= "0000" and s_rd_exmem = s_rs_idex) else
		   "10" when (s_reg_write_memwb = '1' and s_rd_memwb /= "0000" and s_rd_memwb = s_rt_idex) else
		   "00";

    -- MUX FORWARD A (Entrada A da ULA)
    with s_forward_a select
        s_ula_entry_a <= s_a_idex            when "00", -- Sem Forwarding (RegBank)
                         s_ula_out_exmem     when "01", -- Forward de EX/MEM
                         s_ula_out_memwb     when "10", -- Forward de MEM/WB (Resultado ULA)
                         s_mem_out_memwb     when others; -- Fallback: Forward de MEM/WB (Dado Memória)

    -- MUX FORWARD B (Entrada B da ULA) - Recebe a saída do MuxULASrc como entrada "00"
    with s_forward_b select
        s_ula_entry_b_final <= s_ula_entry_b     when "00", -- Sem Forwarding (Saída do MuxULASrc)
                               s_ula_out_exmem   when "01", -- Forward de EX/MEM
                               s_ula_out_memwb   when "10", -- Forward de MEM/WB (Resultado ULA)
                               s_mem_out_memwb   when others; -- Fallback: Forward de MEM/WB (Dado Memória)
    ...
    -- =================================================================================
    -- 5. INSTANCIAÇÕES E CONEXÕES DOS ESTÁGIOS
    -- =================================================================================
    ...
    inst_MuxRegSrc: MuxRegSrc
        port map(
            reg_src => s_reg_src,
            rs      => s_inst_ifid(7 downto 4), -- rt
            rd      => s_inst_ifid(3 downto 0), -- rd
            mux_out => s_dest_addr
        );
    ...
    inst_ID_EX: ID_EX
        port map(
            clock      => clock,
            -- APLICAÇÃO DO FLUSH: reset global OU flush
            reset      => reset or s_id_ex_flush, 
